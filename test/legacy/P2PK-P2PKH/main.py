...
def load_wallet(path=WALLET_JSON):
    ...
    # Per P2PKH abbiamo bisogno di calcolare l'hash160 della chiave pubblica
    # Calcoliamo l'hash160 della chiave pubblica per gli output P2PKH
    h160 = hashlib.new('ripemd160', hashlib.sha256(pub).digest()).digest()
    
    # Creiamo l'indirizzo P2PKH (non necessario per questo script ma utile per debug)
    prefix = b'\x6f' if w.get("network") == "regtest" or w.get("network") == "testnet" else b'\x00'
    addr_payload = prefix + h160
    addr = base58.b58encode(addr_payload + sha256d(addr_payload)[:4]).decode()
...
def main():
    ...
    # 3. Prompt utente
    # Ora chiediamo un indirizzo P2PKH invece di una pubkey
    dest_addr = input("Indirizzo destinatario (P2PKH): ").strip()
    # Decodifichiamo l'indirizzo per ottenere l'hash160
    try:
        # Decodifica Base58Check
        addr_bytes = base58.b58decode(dest_addr)
        # Verifica il checksum
        if sha256d(addr_bytes[:-4])[:4] != addr_bytes[-4:]:
            raise ValueError("Checksum dell'indirizzo non valido")
        # Estrai l'hash160 (salta il primo byte che è il prefisso di rete)
        dest_h160 = addr_bytes[1:-4]
        if len(dest_h160) != 20:
            raise ValueError("Hash160 dell'indirizzo non valido")
    except Exception as e:
        sys.exit(f"Errore nell'indirizzo: {e}")
    ...
    raw,vsize,fee,change_sat,n_in = build_tx(
        ...
        out_build  = SCRIPT_TYPES["p2pkh"]["build"],  # output P2PKH
        chg_build  = SCRIPT_TYPES["p2pk"]["build"],   # resto  P2PK
        sk=sk, pub=pub,
        change_data=pub,                             # Per P2PK usiamo la chiave pubblica
        dest_data=dest_h160
    )

    # 6. Riepilogo
    print("\n··· RIEPILOGO ···")
    print(f"Input usati   : {n_in} (tipo: P2PK)")
    print(f"Fee           : {fee} sat  ({fee/vsize:.2f} sat/vB su {vsize} vB)")
    print(f"Importo out   : {send_sat} sat (tipo: P2PKH)")
    print(f"Indirizzo dest: {dest_addr}")
    print(f"Resto         : {change_sat} sat (tipo: P2PK)")
    print(f"Chiave pubblica resto: {pub.hex()[:10]}...{pub.hex()[-10:]}")
