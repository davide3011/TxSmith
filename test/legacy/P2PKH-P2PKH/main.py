"""Tx legacy P2PKH → P2PKH modulare
=================================================
• Costruisce e (opzionalmente) invia una transazione legacy con input P2PKH
...
def main():
    ...
    spk_hex     = node.validateaddress(addr)["scriptPubKey"]
    ...
    raw,vsize,fee,change_sat,n_in = build_tx(
        ...
        inp_sign   = SCRIPT_TYPES["p2pkh"]["sign"],   # input  P2PKH
        out_build  = SCRIPT_TYPES["p2pkh"]["build"],   # output P2PKH
        chg_build  = SCRIPT_TYPES["p2pkh"]["build"],  # resto  P2PKH
        sk=sk, pub=pub,
        change_data=h160,
        dest_data=dest_h160
    )
